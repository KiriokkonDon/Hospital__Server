...
namespace Hospital.Servise
{
    ...
    {
        private readonly ISpecializationRep _db;

        public SpecializationService(ISpecializationRep db)
        {
            _db = db;
        }
        ...
        {
            if (_db.IsSpecializationExistById(spec.Id))
                return _db.Delete(spec) ? Result.Ok(spec) : Result.Fail<Specialization>("Unable to delete specialization");
